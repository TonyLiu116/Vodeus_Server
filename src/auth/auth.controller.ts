...
export class AuthController {
  ...
  async getPremiumUsers(
    ...
    return await this.usersService.getPremiumUsers()
      .then((data) => res.json(data))
      .catch(err => !err.status ? this.logger.error(err) : res.status(err.status).send(err.response));
  }
  ...
  async getUsers(
    ...
    @Query("filter_country") country: string,
  ): Promise<SubScribeRequest> {
  return await this.usersService.getUsers(page, itemsPerPage, sort, order, search, country)
      .then((data) => res.json(data))
      .catch(err => !err.status ? this.logger.error(err) : res.status(err.status).send(err.response));
  }
  ...
  async getUserTransactionHistory(
    ...
    return this.usersService.getUserTransactionHistory(userId, page, 5, sort, order)
      .then((data) => res.json(data))
      .catch(err => !err.status ? this.logger.error(err) : res.status(err.status).send(err.response));
  }

  /*
  @Get("getuserrecordsnumber")
  @ApiResponse({ status: HttpStatus.OK})
  @ApiQuery({ name: "id", type: String, required: true })
  async getUserRecordsNumber(
    @Res() res,
    @Query("id") userId: string,
  ) {
    return this.usersService.getUserRecordsNumber(userId)
      .then((data) => res.json(data))
      .catch(err => !err.status ? this.logger.error(err) : res.status(err.status).send(err.response));
  }

  @Get("getusertotallitens")
  @ApiResponse({ status: HttpStatus.OK})
  @ApiQuery({ name: "id", type: String, required: true })
  async getUserTotalLitens(
    @Res() res,
    @Query("id") userId: string,
  ) {
    return this.usersService.getUserTotalLitens(userId)
      .then((data) => res.json(data))
      .catch(err => !err.status ? this.logger.error(err) : res.status(err.status).send(err.response));
  }
  */
