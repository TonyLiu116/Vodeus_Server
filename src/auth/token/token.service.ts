...
export class TokenService {
  ...
  async createRefreshToken(tokenContent: { userId; ipAddress: string; }): Promise<string> {
  ...
  async deleteRefreshToken(userId, value: string) {
  ...
  private async isBlackListed(id, expire: number): Promise<boolean> {
    return this.usersExpired[id] && expire < this.usersExpired[id];
  }

  private async revokeTokenForUser(userId): Promise<any> {
