...
export class AuthService {
  ...
  async phoneRegister(phoneNumber) {
  ...
  async confirmPhoneNumber(phoneNumber, verificationCode) {
    ...
    if (exceptUsers.indexOf(phoneNumber) == -1 && (!result.valid || result.status !== 'approved')) {
  ...
  async login(ipAddress, user, isRegister: boolean = false): Promise<LoginResponse> {
    const payload: any = {
  ...
  logout(userId, refreshToken): Promise<any> {
    return this.tokenService.deleteRefreshToken(userId, refreshToken);
  }

  logoutFromAll(userId): Promise<any> {
