...
export class RecordsController {
  ...
  userRecords(
    ...
    const user = req.user;
    return this.recordsService.getRecordsByUser(user.id, skip, take, order, userid) //todo add friend records
  ...
  async addRecord(
    ...
    //this.mailService.sentNotifyToFriends(user.id);
  ...
  async changVoice(
    ...
    const user = request.user;
    return this.recordsService.changeVoiceProfile(body, user, file?.buffer,file?.originalname);
  }
  ...
  allStories(
    ...
    const { id } = req.user;
    return this.recordsService.getRecordsByUser(id, skip, take, order, userId, category, search, recordId, friend, date,targetId)
      .then((data) => {res.json(data);})
      .catch(err => !err.status ? this.logger.error(err) : res.status(err.status).send(err.response));
  }
  ...
  @ApiResponse({ status: HttpStatus.OK, type: [RecordsResponse] })
  @ApiResponse({ status: HttpStatus.UNAUTHORIZED })
  @ApiQuery({ name: 'year', required: true, type: Number, example: 1998})
  ...
  monthStories(
    ...
    const { id } = req.user;
    return this.recordsService.getMonthRecords(id, year, month)
      .then((data) => {res.json(data);})
      .catch(err => !err.status ? this.logger.error(err) : res.status(err.status).send(err.response));
  }
